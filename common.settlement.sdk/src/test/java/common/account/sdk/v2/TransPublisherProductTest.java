 ...
 */
@Slf4j
public class TransPublisherProductTest {
	...
	public static void main(String[] args) {
		...
		req.setOutputAccountNo("1AN2018012200000001"); //入-指定发行人产品户
		List<TradeEvent> eventList = new ArrayList<TradeEvent>();
		TradeEvent tradeEvent = new TradeEvent();
		tradeEvent.setBalance(new BigDecimal(10));
		tradeEvent.setEventType(EventTypeEnum.TRANSFER_PUBLISHER_PRODUCT.getCode());
		eventList.add(tradeEvent);
		req.setEventList(eventList);
		log.info(System.currentTimeMillis()+"");
		AccountTransResponse resp = sdk.transferPublisherProduct(req);
		log.info(System.currentTimeMillis()+"");
		log.info("发行人产品户转账返回：{}",JSONObject.toJSONString(resp));
	}
