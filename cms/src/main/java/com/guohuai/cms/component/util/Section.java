...
public final class Section<T extends Serializable> {
	...
	public Predicate build(Root<T> root, CriteriaBuilder cb, String attributeName) {
		...
		if (this.hasMin) {
			if (this.minEqual) {
				minp = cb.ge(root.get(attributeName).as(this.minType), this.min);
			} else {
				minp = cb.gt(root.get(attributeName).as(this.minType), this.min);
			}
		}

		if (this.hasMax) {
			if (this.maxEqual) {
				maxp = cb.le(root.get(attributeName).as(this.maxType), this.max);
			} else {
				maxp = cb.lt(root.get(attributeName).as(this.maxType), this.max);
			}
		}
