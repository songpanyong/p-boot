...
public class Response extends HashMap<String, Object> {
				...
				if (null != properties && properties.length > 0) {
					...
					for (FieldInfo property : properties) {
						try {
							...
							throw MoneyException.getException(e);
						}
					}
					return response;
				}
	...
	public static Response convert(Object[] values, String[] titles) {
		if (null == values || null == titles || titles.length == 0) {
			throw new MoneyException("Datas and titles can not be null.");
		} else {
			if (values.length != titles.length) {
				throw new MoneyException("Mismatching titles config.");
			} else {
				Response response = new Response();
				for (int i = 0; i < titles.length; i++) {
					if (StringUtil.isEmpty(titles[i])) {
						throw new MoneyException("Empty title at index " + i);
					} else {
						response.with(titles[i].trim(), values[i]);
					}
				}
				return response;
